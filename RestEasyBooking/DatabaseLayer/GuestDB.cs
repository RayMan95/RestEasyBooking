...
namespace RestEasyBooking.DatabaseLayer
{
    ...
    {
        //Data members
        private string tableGuests = "Guests";
        private string sqlLocalGuests = "SELECT * FROM Guests";
        private string tableGuestAccounts = "GuestAccounts";
        private string sqlLocalGuestAccounts = "SELECT * FROM GuestAccounts";
        private string tableRefNums = "ReferenceNumbers";
        private string sqlLocalRefNums = "SELECT * FROM ReferenceNumbers";

        private Collection<Guest> allGuests;
        private Collection<Guest> allGuestAccounts;
        private Collection<Guest> allRefNumbers;

        public GuestDB() : base()
        {
            allGuests = new Collection<Guest>();
            allGuestAccounts = new Collection<Guest>();
            allRefNumbers = new Collection<Guest>();
            FillDataSet(sqlLocalGuests, tableGuests);
            Add2Collection(tableGuests);
            FillDataSet(sqlLocalGuestAccounts, tableGuestAccounts);
            Add2Collection(tableGuestAccounts);
            FillDataSet(sqlLocalRefNums, tableRefNums);
            Add2Collection(tableRefNums);
        }
        ...
        public void Add2Collection(string table)
        {
            ...
            foreach (DataRow myRow_loopVariable in dsMain.Tables[table].Rows)
            {
                ...
                if (!(myRow.RowState == DataRowState.Deleted))
                {
                    ...
                    string guestAccountNumber = Convert.ToString(myRow["GuestAccountNumber"]).TrimEnd();
                    //***The code below shows thee test for database Null values
                    string name = "", phone = "", email= "", address = "";
                    if (myRow["Name"] != System.DBNull.Value) name = Convert.ToString(myRow["Name"]).TrimEnd();
                    if (myRow["Name"] != System.DBNull.Value) phone = Convert.ToString(myRow["Phone"]).TrimEnd();
                    if (myRow["Name"] != System.DBNull.Value) email = Convert.ToString(myRow["Email"]).TrimEnd();
                    if (myRow["Name"] != System.DBNull.Value) address = Convert.ToString(myRow["Address"]).TrimEnd();
